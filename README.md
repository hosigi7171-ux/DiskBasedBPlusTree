...
This can be extended to simple DBMS later.

---

### 프로젝트 설명

이 프로젝트는 in-memory B+Tree를 on disk B+Tree로 전환하는 프로젝트입니다.
이 구현은 DBMS에서 file and index layer, disk space management layer를 구현합니다.
B+Tree 인덱스 구현은 DBMS에서 가장 중요한 작업입니다. 추후에 이 프로젝트는 간단한 DBMS를 구축하는데 확장될 수 있습니다.

다음은 이 프로젝트의 **아키텍처** 입니다. (design.md 참고)
...
레이어드 아키텍처를 적용하여 프로그램을 네 단계로 구분했습니다.
...
1. 디스크에 어떻게 적용할 것인지, 페이지 구조는 알지만 이를 어떻게 해야하는지 비플 트리의 노드를 어떻게 해야하는지
